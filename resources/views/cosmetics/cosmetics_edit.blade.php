...
<meta charset="utf-8">
    ...
    <body>
        ...
        <div class="contents-area register">
            <form action="/cosmetics_edit_confirm/{{ $cosmetic->id }}" method="post" enctype="multipart/form-data">
                @csrf
                
                <p>画像を選択してアップロード</p>
                <input class="btn" type="file" name="image" value="{{ Storage::url($cosmetic->image) }}">
                
                <p>商品名<span class="red">*</span></p>
                @error('name')
                    <span class="red error">{{ $message }}</span>
                @enderror
                <input class="form-control" type="text" name="name" value="{{ $cosmetic->name }}">
                
                <p>種類</p>
                @error('type')
                    <span class="red error">{{ $message }}</span>
                @enderror
                <input class="form-control" type="text" name="type" value="{{ $cosmetic->type }}">

                <p>メーカー名</p>
                @error('company')
                    <span class="red error">{{ $message }}</span>
                @enderror
                <input class="form-control" type="text" name="company" value="{{ $cosmetic->company }}">
                ...
                <div class="flex between">
                    ...
                    @error('get_date')
                        <span class="red error">{{ $message }}</span>
                    @enderror
                    <input class="form-control date-area" type="date" name="get_date" value="{{ $cosmetic->get_date }}">

                    <p>開封日</p>
                    @error('open_date')
                        <span class="red error">{{ $message }}</span>
                    @enderror
                    <input class="form-control date-area" type="date" name="open_date" value="{{ $cosmetic->open_date }}">
                </div>
                ...
                @enderror
                <select name="expiry">
                <option hidden>{{ $cosmetic->expiry }}</option>
                ...
                @error('memo')
                    <span class="red error">{{ $message }}</span>
                @enderror
                <textarea class="form-control" wrap="soft"cols="30" rows="5" name="memo">{{ $cosmetic->memo }}</textarea>

                @if($cosmetic->role == 1)
                <p><input checked="checked" type="checkbox" name="recomend_check" value="recomend">おすすめとして登録されています(チェックを外すと解除されます)</p>
                @else
                ...
                <textarea class="form-control" wrap="soft"cols="30" rows="5" name="comment">{{ $cosmetic->comment }}</textarea>
                @error('comment')
                    <span class="red error">{{ $message }}</span>
                @enderror
                ...
                <div class="btn-area">
                    ...
                    <button class="btn btn-secondary submit-btn" type="submit" onclick="return edit_confirm('この内容で編集しますか？') ">編集する</button>
